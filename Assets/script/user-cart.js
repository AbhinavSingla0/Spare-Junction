...
function showCartProducts() {
  ...
  */
   if (cartProducts !== null && cartProducts.length > 0) {
      ...
      document.querySelector(".user-cart-head").innerHTML = `<tr>
      ...
      document.querySelector(".user-cart-table").innerHTML = cartProducts
         .map((product) => {
      ...
      document.querySelector(".checkout-total-bill").innerHTML =
         totalBill + " Rs.";
      let discountBill = totalBill - (totalBill * 10) / 100; // calculating discount
      document.querySelector(".checkout-discount-bill").innerHTML =
         discountBill + " Rs.";
      ...
      document.querySelector(
         ".user-cart-table"
      ).innerHTML = `<div class="col-lg-12 d-flex justify-content-center">
          <div>
   ...
   document.querySelector(
      ".total-quantity"
   ).innerHTML = `<span>${totalQuantity}</span>`;
}
         ...
         if (buttonType === "add") {
            ...
            if (quantity > 1) {
               ...
               localStorage.setItem(
                  "products",
                  JSON.stringify(cartCurrentProducts)
               );
               ...
               if (cartCurrentProducts.length == 0) {
                  document.querySelector(".user-cart-head").style.display =
                     "none";
               }
            }
         }
...
checkoutbtn.addEventListener("click", () => {
   ...
   for (let i = 0; i < personalInfoFields.length; i++) {
      ...
      if (personalInfoFields[i].value == "") {
         document.querySelector(".cart-modal-title").innerHTML =
            "Please, fill out your shipment information before checkout!";
         isPersonalInfoFilled = false; // if any one field is empty
      }
   }
   ...
   if (isPersonalInfoFilled) {
      document.querySelector(".cart-modal-title").innerHTML =
         "Your order is about to be place!";
   }
